...
public class HomePanel implements WorkPanel{
    ...
    public HomePanel(ControllerUtente controller, UserScene userScene) {
    ...
    public void updateAnnunci() {
        ...
        Map<Integer, String> titolo = controller.getTitoliAnnunci();

        titolo.entrySet().forEach(annuncio -> {
            JPanel titlePanel = new JPanel();
            titlePanel.setBackground(Color.WHITE);
            titlePanel.setLayout(new BoxLayout(titlePanel, BoxLayout.Y_AXIS));

            int codice = annuncio.getKey();
            String categoria = controller.getImmagineCategoria(codice);

            String imagePath;
            imagePath = switch (categoria == null ? "" : categoria.toLowerCase()) {
                case "concerto" -> "img/concerti.png";
                case "escursione" -> "img/escursioni.png";
                case "giro panoramico" -> "img/giroPanoramico.png";
                case "città" -> "img/bus.png";
                default -> "img/BB.png";
            };
            ...
            JButton button = GenericButton.getGenericButton(scaledIcon, dynamicFontSize, "ANNUNCIO");
            button.setAlignmentX(Component.CENTER_ALIGNMENT);
            JLabel codiceLabel = GenericLabel.getGenericLabel(String.valueOf(annuncio.getKey()), dynamicFontSize);
            codiceLabel.setVisible(false);
            codiceLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
            JLabel categoriaLabel = GenericLabel.getGenericLabel(annuncio.getValue(), dynamicFontSize);
            categoriaLabel.setAlignmentX(Component.CENTER_ALIGNMENT);

            int codiceServizio = annuncio.getKey();
