...
public class GiroPanoramicoPanel implements WorkPanel {
    ...
    public GiroPanoramicoPanel(ControllerUtente controller, UserScene userScene) {
    ...
    public void updateGP() {
        ...
        linee.entrySet().forEach(gp -> {
            ...
            ImageIcon icon = new ImageIcon(getClass().getResource("img/giroPanoramico.png"));
            Image scaledImage = icon.getImage().getScaledInstance(imageSize, imageSize, java.awt.Image.SCALE_SMOOTH);
            ImageIcon scaledIcon = new ImageIcon(scaledImage);
            JButton button = GenericButton.getGenericButton(scaledIcon, dynamicFontSize, "GP");
            button.setAlignmentX(Component.CENTER_ALIGNMENT);
            JLabel codiceLabel = GenericLabel.getGenericLabel(String.valueOf(gp.getKey()), dynamicFontSize);
            codiceLabel.setVisible(false);
            codiceLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
            JLabel categoriaLabel = GenericLabel.getGenericLabel(gp.getValue(), dynamicFontSize);
            categoriaLabel.setAlignmentX(Component.CENTER_ALIGNMENT);

            int codiceServizio = gp.getKey();
