...
public class ConcertiPanel implements WorkPanel {
    ...
    public ConcertiPanel(ControllerUtente controller) {
    ...
    public void updateConcerti() {
        ...
        linee.entrySet().forEach(concerto -> {
            ...
            JButton button = GenericButton.getGenericButton("CONCERTO", TEXT_SIZE, "CONCERTO");
            JLabel codiceLabel = GenericLabel.getGenericLabel(String.valueOf(concerto.getKey()), TEXT_SIZE);
            codiceLabel.setVisible(false);
            JLabel titoloLabel = GenericLabel.getGenericLabel(concerto.getValue(), TEXT_SIZE);
